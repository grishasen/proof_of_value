        ...
        if isinstance(value, dict):
            ...
            if key == 'file_type':
                ...
                updated_data[key] = st.checkbox(
                    ...
                    value=strtobool(value), disabled=ro
                )
            elif isinstance(value, numbers.Number):
                updated_data[key] = st.number_input(f"{'###' * level * 2} {name}-{key}", value=value,
                                                    disabled=ro)
            elif isinstance(value, list):
                key_label = f"{'###' * level * 2} {name}-{key}"
                if ro:
                    updated_data[key] = value
                    st.multiselect(label=key_label, options=value, default=value, disabled=ro)
                else:
                    updated_data[key] = st_tags(label=key_label,
                                                text='',
                                                value=value,
                                                key=key_label)
            else:
                if len(str(value)) < 80:
                    updated_data[key] = st.text_input(f"{' ' * level * 2}- {name}-{key}", value, disabled=ro)
                else:
                    updated_data[key] = st.text_area(label=f"{' ' * level * 2}- {name}-{key}", value=value,
                                                     disabled=ro)
...
def display_reports(metrics, reports):
    ...
    for key, report in reports.items():
        other_params = {k: v for k, v in report.items() if k not in ["metric", "type", "description", "group_by"]}
        report_data.append([
            key,
            report.get("metric", ""),
            report.get("type", ""),
    ...
    edited_dict = edited_df[["metric", "type", "description", "group_by", "others"]].to_dict('index')
...
with conf:
    ...
    st.subheader("Branding Section",
                 help="These settings provide versioning and release information about the application.")
    config['copyright'] = display_dict('copyright', config.get('copyright', {}), read_only=False)

    # Display and edit UX section
    st.subheader("UX Section", help="""These settings control the behavior of the application's user interface.
    ...
    **refresh_interval**: The time interval (in milliseconds) for refreshing the dashboard automatically. The default value is 120000, which equals 2 minutes.""")
    ...
    st.subheader("IH Section", help="""These settings define how input data files (usually Interaction History exports) are processed and recognized by the
    application.
    ...
    st.subheader("Metrics Section", help="""The application currently supports various metrics. Please refer to README on metrics' details. 
    
    **Scores** parameter cannot be changed!!!""")
    config['metrics'] = display_dict('metrics', config.get('metrics', {}), read_only=False)

    # Display Reports section (Editable)
    st.subheader("Reports Section", help="""The [reports] section in the configuration file allows for the definition of various analytical reports. 
    ...
    config['variants'] = display_dict('variants', config.get('variants', {}), read_only=False)

    try:
        text = tomlkit.dumps(config)
        st.download_button("Download config file", data=text, file_name="config.toml", type='primary')
    except:
        print_stack()
