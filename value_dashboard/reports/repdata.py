...
@timed
def calculate_reports_data(grouped_rep_data: Union[pd.DataFrame, pl.DataFrame], params: dict) -> pl.DataFrame:
    report_data = None
...
@timed
def group_model_ml_scores_data(model_roc_auc_data: Union[pl.DataFrame, pd.DataFrame],
    ...
    use_t_digest = strtobool(m_config['use_t_digest']) if 'use_t_digest' in m_config.keys() else False
...
@timed
def group_experiment_data(exp_data: Union[pl.DataFrame, pd.DataFrame],
                          config: dict) -> pl.DataFrame:
    if isinstance(exp_data, pd.DataFrame):
        exp_data = pl.from_pandas(exp_data)

    m_config = get_config()["metrics"][config['metric']]
    grp_by = (get_config()["metrics"]["global_filters"] + config['group_by'] + [m_config['experiment_name']])

    grp_by = list(set(grp_by))
    if grp_by:
        exp_data = (
            exp_data
            .group_by(grp_by + [m_config['experiment_group']])
            .agg([
                pl.col("Count").sum(),
...
def calculate_experiment_scores(exp_data: Union[pl.DataFrame, pd.DataFrame],
                                config: dict) -> pl.DataFrame:
    if isinstance(exp_data, pd.DataFrame):
        exp_data = pl.from_pandas(exp_data)
    sort_list = []
    ...
    if grp_by:
        ...
        exp_data = (
            exp_data
            .sort(grp_by + [m_config['experiment_group']], descending=True)
    ...
    if sort_list:
        exp_data = (
            exp_data
            .sort(sort_list, descending=True)
        )

    return exp_data


def calculate_model_ml_scores(model_roc_auc_data: Union[pl.DataFrame, pd.DataFrame],
                              config: dict) -> pl.DataFrame:
    if isinstance(model_roc_auc_data, pd.DataFrame):
        model_roc_auc_data = pl.from_pandas(model_roc_auc_data)
    ...
    auc_data = (
        ...
        .agg(
            ([weighted_mean(pl.col(scores), pl.col("Count")).name.suffix("_a")]
             if not use_t_digest
             else [weighted_mean(pl.col(['personalization', 'novelty']), pl.col("Count")).name.suffix("_a")])
            +
            [
                pl.col("Count").sum().alias("Count_a"),
                pl.col(grp_by).first().name.suffix("_a")
            ...
            (
                [
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True).alias("roc_auc_tdigest_a")
                ] if use_t_digest else []
            )
        )
        .select(cs.ends_with("_a"))
...
def calculate_engagement_scores(ih_analysis: Union[pl.DataFrame, pd.DataFrame],
                                config: dict) -> pl.DataFrame:
    ...
    if grp_by:
        data_copy = (
            ...
            .with_columns(((pl.col("TestCTR") - pl.col("ControlCTR")) / pl.col("ControlCTR")).alias("Lift"),
                          (pl.col("Positives") / pl.col("Negatives")).alias("CTR"))
            .with_columns([
                ((((pl.col("CTR") * (1 - pl.col("CTR"))) /
                   pl.col("Negatives")) ** 0.5)).alias("StdErr"),
                pl.col("Lift").replace({np.inf: 0.0})
            ])
            .with_columns(pl.col("Lift").fill_nan(0.0))
            .with_columns(pl.struct(
                ['Positives_Control', 'Positives_Test', 'Negatives_Control', 'Negatives_Test'])
                          .map_elements(lambda x: proportions_ztest(
                x["Positives_Test"],
                x["Positives_Control"],
                x["Negatives_Test"],
                x["Negatives_Control"]
            ), return_dtype=pl.Struct).alias('Z_Test'))
            .unnest("Z_Test")
            .drop(
                ["TestCTR", "ControlCTR", 'Positives_Control', 'Positives_Test', 'Negatives_Control', 'Negatives_Test'])
...
def calculate_conversion_scores(ih_analysis: Union[pl.DataFrame, pd.DataFrame],
                                config: dict) -> pl.DataFrame:
    if isinstance(ih_analysis, pd.DataFrame):
        ih_analysis = pl.from_pandas(ih_analysis)
...
@timed
def group_engagement_data(eng_data: Union[pl.DataFrame, pd.DataFrame],
                          config: dict) -> pl.DataFrame:
...
@timed
def group_conversion_data(conv_data: Union[pl.DataFrame, pd.DataFrame],
                          config: dict) -> pl.DataFrame:
...
@timed
def group_descriptive_data(data: Union[pl.DataFrame, pd.DataFrame],
                           config: dict) -> pl.DataFrame:
...
def calculate_descriptive_scores(data: Union[pl.DataFrame, pd.DataFrame],
                                 config: dict) -> pl.DataFrame:
    if isinstance(data, pd.DataFrame):
        data = pl.from_pandas(data)
    ...
    if not use_t_digest:
            ...
            .agg(
                ...
                ([
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True
                    ).alias(f"{c}_Median_a") for c in num_columns
                ])
                +
                ([
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True
                    ).alias(f"{c}_p25_a") for c in num_columns
                ])
                +
                ([
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True
                    ).alias(f"{c}_p75_a") for c in num_columns
                ])
                +
                ([
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True
                    ).alias(f"{c}_p90_a") for c in num_columns
                ])
                +
                ([
                    pl.map_groups(
                        ...
                        return_dtype=pl.Struct,
                        returns_scalar=True
                    ).alias(f"{c}_p95_a") for c in num_columns
                ])
            )
        )
        ...
        copy_data = (
            ...
            .with_columns(
                [
                    ((pl.col(f'{c}_p75') + pl.col(f'{c}_p25') - 2 * pl.col(f'{c}_Median')) / (
                            pl.col(f'{c}_p75') - pl.col(f'{c}_p25')))
                    .alias(f'{c}_Skew') for c in num_columns
                ]
            )
            .with_columns(
                [
...
def calculate_clv_scores(exp_data: Union[pl.DataFrame, pd.DataFrame],
                         config: dict) -> pl.DataFrame:
    if isinstance(exp_data, pd.DataFrame):
        exp_data = pl.from_pandas(exp_data)
    m_config = get_config()['metrics'][config['metric']]
    customer_id_col = m_config['customer_id_col'] if 'customer_id_col' in m_config.keys() else CUSTOMER_ID
