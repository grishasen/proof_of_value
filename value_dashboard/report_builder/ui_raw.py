...
def _render_raw_value(label: str, value, key_prefix: str):
    if isinstance(value, list):
        val = st.text_area(label, value="\n".join(str(item) for item in value), key=key_prefix)
        return _parse_list(val)
    if isinstance(value, dict):
        return _render_dict_editor(value, key_prefix)
    if isinstance(value, bool) or isBool(value):
        return st.checkbox(label, value=strtobool(value), key=key_prefix)
    if isinstance(value, int):
        return int(st.number_input(label, value=value, step=1, key=key_prefix))
    if isinstance(value, float):
        return st.number_input(label, value=value, format="%.6f", key=key_prefix)
    value = "" if value is None else str(value)
    if len(value) > 80:
        return st.text_area(label, value=value, key=key_prefix)
    return st.text_input(label, value=value, key=key_prefix)
...
def render_raw_report_editor(cfg: dict, report_name: str, report: dict, original_name: str, key_base: str):
    ...
    name = st.text_input("Report Name", value=report_name, key=f"{key_base}_raw_name")
    ...
    if st.button("Save Report", key=f"{key_base}_raw_save", type="primary"):
        if not name:
